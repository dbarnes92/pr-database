...
SELECT primary_nm, attended_num, wins AS Set_Wins, losses AS Set_Losses, team AS Team, characters as Characters, mu, sigma, DATE_FORMAT(last_match, '%Y-%m-%d') AS Last_Match, decay_months AS Decay_Months
FROM player
Order By wins desc;
...
/* List of tournaments + the tournament winner */
...
- wins in head-to-head table need simply be incremented by 1 for the winner of each set using the player_id's to locate their head-to-head tuple/entry/whatever you address a row of a table as.
    ...
    *^^^This can easily be solved using the OR keyword in the WHERE clause
*/ 